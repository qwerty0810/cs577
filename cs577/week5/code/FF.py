class Node:
    ...
    def findPath(self, dst, visited):
        ...
        for child in self.children:
            path = child.findPath(dst, visited)
            if len(path) > 0:
...
class Graph:
    ...
    def add_edge(self, u, v, w):
        ...
        if self.nodes[v] not in self.nodes[u].children and w > 0:
    ...
    def DFS(self, s, t):
        visited = set()
        reversedPath = self.nodes[s].findPath(t, visited)
        return [i for i in reversed(reversedPath)]

    def FordFulkerson(self, s, t):
        if s == t:
            raise ValueError("Error! Source and sink are the same node!")
        maxFlow = 0
        while True:
            path = self.DFS(s, t)
            ...
            if bottleNeck <= 0:
                raise ValueError("Non-positive bottleneck! Check your computation!")
            maxFlow += bottleNeck
